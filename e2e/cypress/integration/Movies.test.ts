...
describe('Movies view tests', () => {
    ...
    it('Modal open & close', () => {
      ...
      cy.get('[data-testid="movies-wrapper"] > :first-child() [data-testid="open-modal-button"]').should("be.visible").click()
      cy.contains('[data-testid="inner-modal"]', FIRST_MOVIE).should("be.visible")
    ...
    it('Search with records', () => {
      ...
      cy.get('[data-testid="movie-card"]').should('have.length', ALL_MOVIES_PER_PAGE)

      // Send request via search input and wait for response
      cy.intercept('GET', `/${THREE_RECORDS_SEARCH_INPUT}*`).as('getMovies')
      cy.get('[data-testid="search-movies-input"]').type(THREE_RECORDS_SEARCH_INPUT)
      cy.wait("@getMovies")
    ...
    it('Pagination with records', () => {
      cy.get('[data-testid="prev-pagination-btn"]').should('be.visible').and('be.disabled')
      cy.get('[data-testid="next-pagination-btn"]').should('be.visible').and('not.be.disabled')
      cy.get('[data-testid="current-page-number"]').should('be.visible').and('have.text', '1')
      ...
      cy.get('[data-testid="current-page-number"]').should('be.visible').and('have.text', '2')
    ...
    it('Pagination with records and search', () => {
      ...
      cy.get('[data-testid="movies-wrapper"] > :first-child() [data-testid="movie-name"]').contains(TWO_PAGES_SEARCH_INPUT)
      cy.get('[data-testid="next-pagination-btn"]').click()

      cy.get('[data-testid="movie-card"]').should('have.length', SND_PAGE_MOVIES_QTY)
